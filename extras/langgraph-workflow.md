...
│         │ │  "VinFast" query │  │ │  suggest_human (OR)    │     │
...
class AgentState(TypedDict):
    ...
    suggest_human: bool     # (confidence < 7) OR model JSON suggest_human
```
...
- **suggest_human** trong state = `(confidence < 7)` **OR** `suggest_human` từ JSON model
- app.py hiển thị card khi `final_state["suggest_human"]`; tin cũ không có flag thì fallback theo `confidence`
